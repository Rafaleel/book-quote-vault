...
export default function Register() {
  ...
  const handleSubmit = async (e: React.SubmitEvent) => {
    ...
    try {
      ...
      setError(err.response?.data || 'Failed to register. Email might already be in use.');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex flex-col lg:flex-row transition-colors duration-200">
      <div className="hidden lg:flex lg:w-[60%] relative flex-col items-center justify-center p-14 overflow-hidden bg-gradient-to-br from-indigo-950 via-brand-700 to-violet-800">
        ...
        <div className="relative text-center max-w-lg">
          ...
          <p className="mt-6 text-sm text-indigo-300 font-semibold">— J.R.R. Tolkien</p>
        </div>
      ...
      <div className="flex-1 lg:w-[40%] flex items-center justify-center bg-[#f9f8f6] dark:bg-slate-950 px-8 py-14 transition-colors duration-200">
        <div className="w-full max-w-[360px]">
          <div className="mb-8">
            <h1 className="text-2xl font-bold font-serif text-slate-900 dark:text-white tracking-tight">Create an account</h1>
            <p className="text-sm text-slate-400 dark:text-slate-500 mt-1.5">Start building your library today.</p>
          </div>
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            <div>
              ...
              <input id="email" name="email" type="email" required value={email} onChange={e => setEmail(e.target.value)} className={inputClass} placeholder="name@example.com" />
            </div>
            <div>
              <label htmlFor="password" className={labelClass}>Password</label>
              <input id="password" name="password" type="password" required value={password} onChange={e => setPassword(e.target.value)} className={inputClass} placeholder="At least 6 characters" />
            </div>
            <div>
              <label htmlFor="confirmPassword" className={labelClass}>Confirm password</label>
              <input id="confirmPassword" name="confirmPassword" type="password" required value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} className={inputClass} placeholder="••••••••" />
            </div>
            <button type="submit" disabled={isLoading} className="w-full py-2.5 bg-brand-600 hover:bg-brand-700 text-white text-sm font-semibold rounded-lg transition-colors disabled:opacity-60">
          ...
          <p className="mt-6 text-center text-xs text-slate-400 dark:text-slate-550">
            ...
            <Link to="/login" className="font-bold text-brand-600 dark:text-brand-450 hover:text-brand-700 dark:hover:text-brand-350 transition-colors">Sign in</Link>
          </p>
        </div>
      </div>
    </div>
